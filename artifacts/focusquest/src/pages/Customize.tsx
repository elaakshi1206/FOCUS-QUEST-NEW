...
import { Lock, UserCheck, Shirt, Rocket, Gem, TrendingUp, Award, Target } from 'lucide-react';
import confetti from 'canvas-confetti';

type Tab = 'characters' | 'outfit' | 'vehicle' | 'accessory' | 'progress';
        ...
        <div className="flex justify-center gap-4 mb-8">
          {WORLD_TABS.map(w => (
            <button key={w.id} onClick={() => setSelectedWorld(w.id as any)}
              className={`relative w-28 h-28 sm:w-32 sm:h-32 rounded-2xl flex flex-col items-center justify-center gap-2 font-display font-bold transition-all duration-300 ${selectedWorld === w.id ? `bg-gradient-to-br ${w.bg} text-white scale-110 shadow-[0_0_20px_rgba(255,255,255,0.4)] ring-4 ring-white z-10` : 'bg-black/30 backdrop-blur-md border border-white/10 text-white/60 hover:bg-white/10 hover:scale-105'}`}>
              <span className="text-4xl sm:text-5xl drop-shadow-md">{w.emoji}</span>
              <span className="text-xs sm:text-sm tracking-wide">{w.label}</span>
            </button>
          ))}
        </div>
